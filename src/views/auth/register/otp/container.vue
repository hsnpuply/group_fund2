...
// Read phone and name from route params or query
const phone = ref<string>(
  (route.params.phone as string) || (route.query.phone as string) || ""
);

const fullName = ref<string>((route.query.name as string) || "");

const inputs = ref<string[]>(Array.from({ length: 5 }).map(() => ""));
...
onMounted(() => {
  // Redirect back to register if phone is not provided
  if (!phone.value) {
    ...
    <div class="otp-container">
      ...
      <div class="header-section">
        <div class="icon-wrapper">
          <v-icon size="48" color="#4caf50">mdi-message-text-lock</v-icon>
        </div>
        <h1 class="title">تایید شماره موبایل</h1>
        <p class="subtitle">
          کد ۶ رقمی ارسال شده به شماره
          <span class="phone-highlight" dir="ltr">{{ phone }}</span>
          را وارد کنید
        </p>
        <p class="user-name" v-if="fullName">
